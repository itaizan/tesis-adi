...
//        }
    ...
    /* @override */
     public void paintComponent(Graphics g) {
        ...
        //revisi monitoring waktu per jam
        //cek dulu apakah udah ganti jam atau belum
        int hourNow = (int) (JistAPI.getTime() / Constants.HOUR);
        if (hourNow != hourLast)
        {
            hourLast = hourNow;
            System.out.println("WRITE CSV (" + hourNow + "):");
            writeCsv = true;
            toCSV = String.valueOf(hourNow);
        }
        ...
        if (statEntryList != null)
            for (StatEntry statEntry:statEntryList) {
                ...
                if (writeCsv) //udah ganti jam, build string untuk dimasukan ke csv
                    if (!statEntry.getHeader().toUpperCase().contains("TIME")) //informasi Time saya abaikan
                        toCSV = toCSV + "," + statEntry.getValueAsString();
        ...
        if (writeCsv) {
            writeCsv = false;
            String csvFile = "OutputHourMonitor.csv"; //nama file output
            try {
                CSVWriter writer = new CSVWriter(new FileWriter(csvFile, true));
                if (hourNow == 0) { //run ke 0 bikin header di csv
                    String[] header = "Hour,NCA_AlivesNodeCount,NCD_DeadNodesCount,AV_Created,AV_Received,AV_Ratio,ALL_EnergyLeft,PerRegion_EnergyLeft (1-3)".split(",");
                    writer.writeNext(header);
                }
                //selanjutnya masukin ini aja, hasil string builder sebelumnya ke csv
                String[] record = toCSV.split(",");
                writer.writeNext(record);
                writer.close();

            } catch (IOException ex) {
                java.util.logging.Logger.getLogger(StatsCollector.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
            }
        }
